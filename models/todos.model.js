...
export default class ToDosModel {
	...
	static getToDos = async () => {
		...
		return db.getDb().collection(Constants.TODOS_COLLECTION).find({},
			{ projection: Constants.DEFAULT_PROJECTION }).toArray();// this is returning a promise and that goes all the way up to the controller that is awaiting that promise
	...
	static getToDo = (id) => {
		...
		// return todo;
		return db.getDb().collection(Constants.TODOS_COLLECTION).findOne({ "id": id }, { projection: Constants.DEFAULT_PROJECTION });
	};
